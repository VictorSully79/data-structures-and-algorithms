...
const count = (target, input) => {
  ...
  const arraysForDays = input.reduce((acc, val, idx) => {
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  console.log(input);
  const feelThePowerOfTheTwo = [];
  input.filter(value => {
    feelThePowerOfTheTwo.push(value.filter(newVal => {
      return newVal %5 === 0 && typeof(newVal) === 'number';

    }))
  })
  return feelThePowerOfTheTwo.map(value => {
...
let findShortest = (data) => {
  console.log(data);
  // Solution code here...
  let heyShorty = Number.MAX_VALUE;
  data.reduce((acc, val, idx) => {
    heyShorty = Math.min(val.height, heyShorty);
  }, 0)
  return data.filter(character => {
    return Number(character.height) === heyShorty;
  })[0].name;
};
