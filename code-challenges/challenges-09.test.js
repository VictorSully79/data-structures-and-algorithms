...
function transformToLis(obj){
  ...
  for(let value in obj){
    deconstructed.push(`<li>${value}: ${obj[value]}</li>`);
  }
...
const addPurchases = (arr) => {
  // Solution code here...
  console.log(arr);
  const totalBill = arr.map(value => value.purchasePrice);
    return totalBill.reduce((acc, curr) => {return acc + curr}, 0);
...
const returnNames = (arr) => {
  // Solution code here...
  console.log(arr);
  const returnOfTheNamesPartDeux = arr. reduce((array, value) => {
    array.push(value.name);
    return array;
  }, [])

  return returnOfTheNamesPartDeux; 
};
...
const reversedString = (str) => {
  // Solution code here...
  console.log(str);
  const rewind = str.split('');
  return rewind.reduceRight((theArray, theChar) => {
